...
def calculate_psnr_from_tensor(target, ref, data_range=1.0):
    '''
    target, ref: (N,C,H,W)
    '''
    n,v,h,w = target.shape
    target = target.view(n,-1).cpu().detach().clamp(0, 1).numpy()
    ref = ref.view(n,-1).cpu().detach().clamp(0, 1).numpy()
    diff =  target - ref
